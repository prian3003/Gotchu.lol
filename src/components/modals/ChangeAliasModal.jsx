...
const ChangeAliasModal = ({ isOpen, onClose, user, onAliasChange }) => {
  ...
  const validateAlias = (name) => {
    if (name.length > 20) return 'Alias must be less than 20 characters'
    if (!/^[a-zA-Z0-9_]*$/.test(name)) return 'Alias can only contain letters, numbers, and underscores'
    if (name === user?.alias) return 'This is your current alias'
    return ''
  }
    ...
    <ModalOverlay onClick={onClose}>
      <ModalContent colors={colors} onClick={e => e.stopPropagation()}>
        ...
        <ModalBody>
          ...
          {/* Info */}
          <InfoText>
            Alias must be unique (0-20 chars) and can only contain letters, numbers, and underscores.
          </InfoText>
        </ModalBody>
      </ModalContent>
    </ModalOverlay>
